...
public abstract class GameCharacter {
    ...
    public void checkScreenBounds() {
        if (position.x + (float) texture.getWidth() / 2 > 1280.0f) position.x = 1280.0f - (float) texture.getWidth() / 2;
        if (position.x - (float) texture.getWidth() / 2 < 0.0f) position.x = 0.0f + (float) texture.getWidth() / 2;
        if (position.y + (float) texture.getHeight() / 2 > 720.0f) position.y = 720.0f - (float) texture.getHeight() / 2;
        if (position.y - (float) texture.getHeight() / 2 < 0.0f) position.y = 0.0f + (float) texture.getHeight() / 2;
    }
    ...
    public void render(SpriteBatch batch, BitmapFont font) {
        ...
        font.draw(batch, stringHelper, position.x - 40, position.y + 80 - 25 + (int) (Math.sin(animationTimer * 10) * 15 * damageEffectTimer), 80, 1, false);
    }
